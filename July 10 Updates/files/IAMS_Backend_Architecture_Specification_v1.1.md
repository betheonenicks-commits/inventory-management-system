...
Document ID: IAMS-BAS-1.1 | Version: 1.1 | Status: Reconciled to Baseline v2.0 — Ready for Engineering | Date: 2026-07-10

Related Documents: IAMS-BRD-2.0 (Business Requirements), IAMS-FRS-2.0 (Functional Requirements), IAMS-SRS-2.0 (Architecture / NFR / Security), IAMS-PUC-1.1 (Personas & Use Cases)

> **v1.1 (2026-07-10):** references re-pointed to the consolidated baseline (BRD 2.0 / FRS 2.0 / SRS 2.0 / PUC 1.1); corrected stale FR-ID citations per FRS 2.0 Appendix B (FR-AST-13→FR-AST-14 insurance, FR-AST-14→FR-AST-15 vehicle, FR-AST-15→FR-AST-16 depreciation, FR-USR-05→FR-USR-08 offboarding, FR-USR-07→FR-USR-09 SoD waivers); flagged the MinIO object-store addition (SRS 2.0 §2.4) against the file-storage section (Section 10). Content otherwise unchanged.
...
Purpose: dedicated, indexed insurance columns per Decision #6 / FR-AST-14 — contrasted with JSONB custom fields in Section 4.
...
| audit_finding_id | UUID | FK `audit_finding.id`, nullable — links claim to the damage/loss finding that triggered it (FR-AST-14) |
...
### `vehicle_detail` (R2)
Purpose: dedicated, indexed Vehicle subtype columns per Decision #6 / FR-AST-15.
...
Purpose: FR-AST-16/FR-CMP-07 depreciation calculation (GAAP/IFRS-aligned methods).
...
### `sod_waiver` (FR-USR-09) — see Section 5.
...
### `offboarding_case` / `offboarding_asset_action` (FR-USR-08)
...
| `EmployeeOffboardingInitiatedEvent` | USR | LIF | Triggers the assigned-asset review gate (FR-USR-08) — LIF exposes `AssetReassignmentRequiredQuery`, USR blocks deactivation until it returns empty |
...
> **Reconciliation flag (v1.1):** SRS 2.0 §2.4/§2.5 formalizes this object store as a new mandatory deployment container, `iams-objectstore` (MinIO), alongside `iams-backend`/`iams-frontend`/`iams-db`/`iams-reverse-proxy`, and confirms the same rules this section already assumes (object-key-only persistence in the database, backend-brokered access, no direct client reachability). This document has no deployment/container topology section of its own to update, but implementers should note per SRS 2.0 §6 that on-premises backup scope must now explicitly cover the `iams-objectstore` volume, not just `iams-db`.
...
| End-to-end | Playwright (frontend-owned, out of this document's scope) | Full user journeys per IAMS-PUC-1.1 use cases |
