...
| — | **SoD domain-check** | *Not a filter* — a service-layer policy evaluation (Section 3.3) | Deliberately excluded from the generic filter chain because it requires loading the specific target entity (e.g., "did this user submit this audit?"), which isn't resolvable from the URL alone | FR-USR-06/07, FR-AUD-22 |
...
3. **Separation of Duties** — for the specific subset of actions FR-USR-06/FR-AUD-22 guard, is the acting user also the entity's creator/submitter, and if so, does an active waiver (FR-USR-07) cover this action type in this scope?
...
public class AccessPolicyEvaluator {
    public boolean can(Authentication auth, String permission, ScopedEntityRef target) {
        ...
        if (sod.isGuardedAction(permission)) {                                    // layer 3
            ...
            if (isSelfApproval) {
                auditLog.recordViaWaiver(auth, target, permission); // FR-USR-07: logged distinctly
            }
        }
        return true;
    }
...
Per SRS 2.3, the Integrations module (INT) is "the single point through which any external system communicates with IAMS" — implemented as a hexagonal adapter layer so every cross-boundary control (FR-SEC-09/10/14, FR-INT-05, BRD 6.5) is enforced **once**, in the gateway, not re-implemented per integration:
...
## 8.3 Outbound Webhooks: Allow-List + HMAC Signing (FR-SEC-10)

- **Allow-list:** `webhook_allowlist` table, Administrator-managed only, **step-up required** to add/change an entry (Section 3.4) — this is the SSRF prevention control (SRS 6.6).
...
Ties directly to FR-SEC-04's immutable security-event log — this is the runbook Officer Reyes (IT Security Officer, IAMS-PUC-1.0) actually uses, following the exact investigation pattern in UC-SEC-01.
...
|---|---|
| 1. Filter Chain | NFR-SEC-01/02, NFR-OBS-03, NFR-MAINT-04, NFR-API-01/02, FR-SEC-01, FR-USR-06/07, FR-AUD-22 |
| 2. AuthN | FR-SEC-01, FR-SEC-02, FR-SEC-03, FR-SEC-05, NFR-SEC-01, SRS 6.1, SRS 6.6 (LDAP/SSO rows) |
| 3. AuthZ | NFR-SEC-02, FR-USR-03/04/06/07, FR-AUD-22, FR-SEC-06, SRS 6.2 |
| 4. CORS/CSRF | SRS 6.7, OWASP ASVS V4 |
| 5. Secrets | FR-SEC-11, NFR-DEPLOY-03, SRS 6.5 |
...
| 8. Integration Gateway | FR-SEC-09/10/14, FR-INT-01–05, SRS 6.6, BRD 6.4/6.5 |
