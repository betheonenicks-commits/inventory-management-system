...
Document ID: IAMS-API-1.0 | Version: 1.0 | Status: Draft for Review
...
| PATCH | `/assets/{id}/insurance` | Set/update dedicated insurance columns (insurer, policy #, coverage, expiry) | INVENTORY_MANAGER, ADMIN | R1 | FR-AST-13 |
| PATCH | `/assets/{id}/vehicle-attributes` | Set/update dedicated Vehicle subtype columns (VIN, registration, odometer, expiry) | INVENTORY_MANAGER, ADMIN | R1 | FR-AST-14 |
| GET | `/assets/{id}/depreciation` | Computed depreciation schedule for one asset | broad read roles | R2 | FR-AST-15 |

**Judgment Call**: FR-AST-01–14 (registration, categorization, attachments, custom fields, history, insurance, vehicle attributes) ship in R1 — they're required to produce the R1 "basic asset register" (BRD 8.2). FR-AST-15 (depreciation calculation) ships in R2 because its only consumer, FR-RPT-09 (Depreciation report), is explicitly R2 per the FRS Section 5 release-mapping appendix; shipping the calculation engine a release early with no report to expose it through has no user value.
...
| POST | `/assets/{id}/return-to-inventory` | Return an assigned asset to unassigned inventory (used by offboarding, Section 4.5) | INVENTORY_MANAGER, ADMIN | R1 (needed for offboarding, FR-USR-05) | FR-LIF-04, FR-USR-05 |
...
**Judgment Call**: `POST /assets/{id}/return-to-inventory` is pulled forward to R1 even though the rest of module LIF is R2, because FR-USR-05 (Offboarding, R1 per BR-21's R1 scope and the fact user management is core R1) requires *some* way to detach an asset from a deactivated user, and R1 has no full Transfer/Assignment workflow yet. In R1, this endpoint performs the minimal unassign-and-flag-unassigned operation; the richer Transfer/Assignment approval workflow (FR-LIF-04/05) supersedes it in R2.
...
| POST | `/users/{id}/deactivate` | Offboard: blocked while assets remain assigned (Section 4.5) | ADMIN, SUPER_ADMIN | R1 | FR-USR-05 |
...
| GET | `/sod-waivers` | List Separation-of-Duties waivers | ADMIN, SUPER_ADMIN, IT_SECURITY_OFFICER | R1 | FR-USR-07 |
| POST | `/sod-waivers` | Record a waiver (Section 4.4) | SUPER_ADMIN | R1 | FR-USR-07 |
| POST | `/sod-waivers/{id}/revoke` | Revoke an active waiver | SUPER_ADMIN | R1 | FR-USR-07 |
...
| GET/POST | `/integrations/webhooks` | List/register outbound webhook subscriptions (allow-listed URL only) | SUPER_ADMIN | R3 | FR-INT-04, FR-SEC-10 |
...
## 4.5 Offboarding with Blocking Asset List (FR-USR-05, UC-USR-01)
...
`X-IAMS-Signature` is HMAC-SHA256 over the **raw request body bytes**, keyed with the per-webhook signing secret (FR-SEC-10); the receiving system recomputes and compares using a constant-time comparison, per standard webhook-security practice. Delivery uses exponential backoff on non-2xx responses, capped at a configurable retry count; every attempt (including failures) is recorded and visible at `GET /integrations/webhooks/{id}/deliveries`. Supported `event` values (initial set): `audit.completed`, `asset.status_changed`.
...
3. **`return-to-inventory` pulled into R1** ahead of the rest of module LIF, to make FR-USR-05 offboarding actually functional in R1. Confirm the minimal R1 semantics (unassign-only, no approval workflow) are acceptable versus deferring offboarding's asset-recovery step entirely to R2.
